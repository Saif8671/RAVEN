...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .section-title {
    ...
    font-weight: 700; line-height: 1.1; margin-bottom: 60px; max-width: 600px;
  }
  .features-grid {
  ...
  .scan-header { text-align: center; margin-bottom: 60px; }
  .scan-form-wrap {
  ...
  .form-label {
    ...
    color: var(--text3); text-transform: uppercase; display: block; margin-bottom: 10px;
  }
  .form-input {
  ...
  .scanning-wrap {
    width: 100%; max-width: 640px;
    background: var(--surface); border: 1px solid var(--border2);
    padding: 48px; border-radius: 4px; text-align: center;
  }
  .scan-radar {
  ...
  @keyframes scanring {
    0%{opacity:0;transform:scale(0.6)} 40%{opacity:0.5} 100%{opacity:0;transform:scale(1.2)}
  }
  .scan-dot {
  ...
  .scan-agents { margin-top: 36px; text-align: left; }
  .scan-agent {
  ...
  .agent-status.done { background: var(--teal); box-shadow: 0 0 8px var(--teal); }
  .agent-status.active { background: var(--accent); box-shadow: 0 0 8px var(--accent); animation: pulse 1s infinite; }
  .agent-status.pending { background: var(--text3); }
  .agent-name { flex: 1; color: var(--text2); }
  .agent-name.done { color: var(--text); }
  .agent-tag {
    font-size: 10px; letter-spacing: 0.1em; padding: 3px 8px;
    border-radius: 2px;
  }
  .agent-tag.done { background: rgba(29,184,138,0.12); color: var(--teal); }
  .agent-tag.active { background: rgba(232,84,58,0.12); color: var(--accent); }
  .agent-tag.pending { background: var(--surface2); color: var(--text3); }

  /* ── RESULTS ───────────────────────────── */
  .results-page { min-height: 100vh; padding: 120px 80px 80px; }
  .results-top {
    display: grid; grid-template-columns: 300px 1fr; gap: 32px; margin-bottom: 40px;
  }
  .score-card {
    background: var(--surface); border: 1px solid var(--border2);
    padding: 40px 32px; border-radius: 4px; text-align: center;
    position: relative; overflow: hidden;
  }
  .score-card::before {
    content: ''; position: absolute; top: -1px; left: 0; right: 0; height: 2px;
  }
  .score-card.critical::before { background: #e24b4a; }
  .score-card.high::before { background: var(--accent); }
  .score-card.medium::before { background: var(--gold); }
  ...
  .score-number {
    font-family: var(--font-head); font-size: 88px; font-weight: 800;
    line-height: 1; margin-bottom: 8px;
  }
  .score-number.critical { color: #e24b4a; }
  ...
  .score-breakdown { display: flex; flex-direction: column; gap: 10px; text-align: left; }
  .score-row {
    display: flex; justify-content: space-between; align-items: center;
    font-family: var(--font-mono); font-size: 12px;
    padding: 8px 12px; background: var(--bg2); border-radius: 3px;
  }
  .score-row-label { color: var(--text2); }
  .score-row-val { font-weight: 500; }
  .score-row-val.critical { color: #e24b4a; }
  .score-row-val.high { color: var(--accent); }
  .score-row-val.medium { color: var(--gold); }
  ...
  .summary-text { color: var(--text2); line-height: 1.7; margin-bottom: 28px; }
  ...
  .sev-badge.Critical { background: rgba(226,75,74,0.15); color: #e24b4a; border: 1px solid rgba(226,75,74,0.3); }
  .sev-badge.High     { background: rgba(232,84,58,0.15); color: var(--accent2); border: 1px solid rgba(232,84,58,0.3); }
  .sev-badge.Medium   { background: rgba(240,165,0,0.12); color: var(--gold); border: 1px solid rgba(240,165,0,0.3); }
  .sev-badge.Low      { background: rgba(29,184,138,0.10); color: var(--teal); border: 1px solid rgba(29,184,138,0.3); }
  .finding-title { flex: 1; font-size: 15px; font-weight: 500; }
  .finding-chevron {
  ...
  .finding-plain {
    padding: 16px; background: var(--surface); border-radius: 3px;
  ...
  .fix-step {
    display: flex; gap: 14px; font-size: 14px; color: var(--text2);
    line-height: 1.5;
  }
  .fix-step-num {
    font-family: var(--font-mono); font-size: 11px;
  ...
  /* ── INCIDENT PAGE ─────────────────────── */
  .incident-page { min-height: 100vh; padding: 120px 80px 80px; }
  .incident-header { margin-bottom: 60px; }
  .incident-warning {
    display: inline-flex; align-items: center; gap: 10px;
    background: rgba(226,75,74,0.1); border: 1px solid rgba(226,75,74,0.3);
    padding: 10px 20px; border-radius: 3px; margin-bottom: 32px;
    font-family: var(--font-mono); font-size: 12px; color: #e24b4a;
    letter-spacing: 0.08em;
  }
  .incident-form-wrap {
    max-width: 640px;
    background: var(--surface); border: 1px solid rgba(226,75,74,0.3);
    padding: 48px; border-radius: 4px;
    position: relative;
  }
  .incident-form-wrap::before {
    content: ''; position: absolute; top: -1px; left: 0; right: 0; height: 2px;
    background: #e24b4a;
  }
  .playbook-wrap {
  ...
  .playbook-steps { display: flex; flex-direction: column; gap: 20px; }
  .playbook-step {
  ...
  .playbook-step-time {
    font-family: var(--font-mono); font-size: 11px;
    color: #e24b4a; letter-spacing: 0.08em;
    padding-top: 2px;
  }
  ...
  .playbook-step-desc { font-size: 14px; color: var(--text2); line-height: 1.65; }
  .playbook-connector {
  ...
  .breach-banner {
    ...
    padding: 20px 24px; border-radius: 3px; margin-bottom: 24px;
    display: flex; align-items: flex-start; gap: 16px;
  }
  .breach-icon { font-size: 20px; flex-shrink: 0; padding-top: 2px; }
  .breach-text { font-size: 14px; color: var(--text2); line-height: 1.65; }
  .breach-text strong { color: #e24b4a; }
`;
